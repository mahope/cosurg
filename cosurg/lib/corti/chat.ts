...
function buildPrompt({ question, lang, recap, patientContext, grounding, caseMode }: ChatRequest): string {
  ...
  if (caseMode) {
    lines.push(
      ...
      "- Add '**Røde flag:**' (2-4 items, a few words each) when can't-miss findings are relevant to this injury.",
      "- '**Behandling:**' as numbered steps, ONE short line per step, concrete (agent, dressing, dose where sourced).",
      "  At most ONE line each of '**Pitfalls:**' and '**Opfølgning:**' — only when they change the plan.",
