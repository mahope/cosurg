...
| **Agentic framework** | [`lib/corti/agent.ts`](cosurg/lib/corti/agent.ts) | Five agents with schema connectors and structured output: answer interpreter and note writer here, an intent router ([`app/api/route/agent.ts`](cosurg/app/api/route/agent.ts)), a topic router for the treatment lookup ([`app/api/guide/route.ts`](cosurg/app/api/guide/route.ts)), and the clinical lookup agent ([`lib/corti/chat.ts`](cosurg/lib/corti/chat.ts)), which has Corti's registry experts and our own MCP server attached as connectors. |
